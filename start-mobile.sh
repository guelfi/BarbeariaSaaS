...
# Script para gerenciar o Web.Mobile (React PWA + Vite)

PORT=4003
PROJECT_NAME="Web.Mobile"
PROJECT_PATH="src/Web.Mobile"
PID_FILE="logs/mobile.pid"
...
start() {
    ...
    npm run dev > "../logs/mobile-stdout.log" 2> "../logs/mobile-stderr.log" &
    pid=$!
    
    echo $pid > "../$PID_FILE"
    
    echo "⏳ Aguardando o Mobile inicializar..."
    sleep 5

    if kill -0 "$pid" 2>/dev/null; then
        echo "✅ $PROJECT_NAME iniciado com sucesso (PID: $pid)."
        echo "📝 Logs disponíveis em: logs/mobile-stdout.log e logs/mobile-stderr.log"
    else
