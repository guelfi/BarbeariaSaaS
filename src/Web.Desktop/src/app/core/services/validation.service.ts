...
export class ValidationService {
  ...
  static passwordStrengthValidator(): ValidatorFn {
    return (control: AbstractControl): ValidationErrors | null => {
      ...
      if (password.length < 8) {
        errors.minLength = { message: 'Senha deve ter pelo menos 8 caracteres' };
      }

      // Check for uppercase letter
      if (!/[A-Z]/.test(password)) {
        errors.uppercase = { message: 'Senha deve conter pelo menos uma letra maiúscula' };
      }

      // Check for lowercase letter
      if (!/[a-z]/.test(password)) {
        errors.lowercase = { message: 'Senha deve conter pelo menos uma letra minúscula' };
      }
      ...
      if (!/[!@#$%^&*(),.?":{}|<>]/.test(password)) {
        errors.specialChar = { message: 'Senha deve conter pelo menos um caractere especial' };
      }
  ...
  // CEP validation for Brazilian format
  static cepValidator(): ValidatorFn {
    return (control: AbstractControl): ValidationErrors | null => {
      ...
      const valid = cepRegex.test(control.value);
